...
export class DesignSystemArchitect {
    ...
    timeoutMs = 200; // Total budget for all specialists
    colorScientist;
    typographyArchitect;
    spatialEngineer;
    constructor() {
    ...
    async run(dna, opts) {
        ...
        try {
            ...
            throw new Error(`DesignSystemArchitect failed: ${error.message}`);
        }
    }
    /**
     ...
     */
    synthesize(synthesis, opts, cacheKey) {
        const diagnostics = {
            notes: [],
            audits: [],
            timings: {},
     ...
     */
    generateCSSVariables(tokens, typeFacts, spatialFacts) {
        const lines = ['@theme {'];
        // Colors
     ...
     */
    generateTailwindV4CSS(tokens, typeFacts, spatialFacts) {
     ...
     */
    generateAnimation(reducedMotion) {
     ...
     */
    generateDocs(tokens, colorFacts, typeFacts, spatialFacts) {
 ...
 */
export function createDesignSystemArchitect() {
    return new DesignSystemArchitect();
}
//# sourceMappingURL=DesignSystemArchitect.js.map