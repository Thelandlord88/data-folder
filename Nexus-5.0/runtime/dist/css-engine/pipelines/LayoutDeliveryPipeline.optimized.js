...
export class LayoutDeliveryPipeline {
     ...
     */
    async generateLayoutMatrix(dna, config = {}) {
     ...
     */
    computeBreakpointMetrics(bp, baseUnit) {
     ...
     */
    generateDefaultBreakpoints(strategy, config) {
        ...
        if (strategy === 'fixed') {
            ...
            return defaults.map(bp => {
                ...
                if (fluidClamp) {
                    if (fluidClamp.gutterMin !== undefined)
                        gutter = Math.max(gutter, fluidClamp.gutterMin);
                    if (fluidClamp.gutterMax !== undefined)
                        gutter = Math.min(gutter, fluidClamp.gutterMax);
                    if (fluidClamp.marginMin !== undefined)
                        margin = Math.max(margin, fluidClamp.marginMin);
                    if (fluidClamp.marginMax !== undefined)
                        margin = Math.min(margin, fluidClamp.marginMax);
                }
     ...
     */
    integrateLayoutIntoDNA(dna, matrix) {
        return {
            ...dna,
            layout: {
    ...
    validateMatrix(matrix) {
        ...
        for (let i = 1; i < matrix.breakpoints.length; i++) {
            if (matrix.breakpoints[i].width <= matrix.breakpoints[i - 1].width) {
                errors.push(`Breakpoint ${matrix.breakpoints[i].name} width must be greater than ${matrix.breakpoints[i - 1].name}`);
            }
        }
 ...
 */
export function createLayoutPipeline() {
    return new LayoutDeliveryPipeline();
}
// Export singleton instance
export const layoutPipeline = new LayoutDeliveryPipeline();
//# sourceMappingURL=LayoutDeliveryPipeline.optimized.js.map