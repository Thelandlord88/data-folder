...
export class ConversationHearing {
    ...
    detectEmotion(text) {
        ...
        for (const [emotionType, config] of Object.entries(emotionalMarkers)) {
            const matches = config.patterns.filter(pattern => text.toLowerCase().includes(pattern.toLowerCase()));
            if (matches.length > 0) {
    ...
    detectCognitivePatterns(text) {
        ...
        for (const [patternType, config] of Object.entries(cognitiveIndicators)) {
            const matches = config.patterns.filter(pattern => text.toLowerCase().includes(pattern));
            if (matches.length > 0) {
    ...
    detectMetaAwareness(text) {
        ...
        const detectedMeta = metaPatterns.filter(pattern => text.toLowerCase().includes(pattern));
        return {
    ...
    assessCognitiveComplexity(text, patterns) {
        ...
        const complexityCount = complexityIndicators.filter(indicator => text.toLowerCase().includes(indicator)).length;
    ...
    classifyInteractionType(dynamics) {
        if (dynamics.invitation)
            return 'collaborative_invitation';
        if (dynamics.building)
            return 'idea_building';
        if (dynamics.questioning)
            return 'exploratory_questioning';
        if (dynamics.validation)
    ...
    classifyBreakthroughType(indicators) {
        ...
        if (types.includes('paradigm_shift'))
            return 'paradigm_shift';
        if (types.includes('major_breakthrough'))
            return 'major_breakthrough';
        if (types.includes('mind_blown'))
            return 'cognitive_surprise';
...
//# sourceMappingURL=conversation-hearing.mjs.map