 ...
 */
export async function getPreviewPage(req: IncomingMessage, res: ServerResponse): Promise<void> {
  try {
 ...
 */
export async function testLayoutSystem(req: IncomingMessage, res: ServerResponse): Promise<void> {
  try {
    const url = new URL(req.url || '', `http://${req.headers.host}`);
    const strategy = url.searchParams.get('strategy') || 'hybrid';
    const baseUnit = parseInt(url.searchParams.get('baseUnit') || '8');
 ...
 */
function generateRecommendations(matrix: any, validation: any): string[] {
  ...
  matrix.breakpoints.forEach((bp: any) => {
    const colWidth = matrix.computed[bp.name].columnWidth;
    if (colWidth < 40) {
  ...
  // Check for clamping
  const clampedBreakpoints = matrix.breakpoints.filter((bp: any) => 
    matrix.computed[bp.name].clamped
  );
  if (clampedBreakpoints.length > 0) {
    recommendations.push(`ℹ️  Clamping applied to: ${clampedBreakpoints.map((bp: any) => bp.name).join(', ')}. This is normal for extreme viewports.`);
  }
