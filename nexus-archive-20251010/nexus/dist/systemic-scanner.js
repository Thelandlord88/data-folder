...
export class SystemicScanner {
    projectRoot;
    cfg;
    constructor(projectRoot = process.cwd(), cfg) {
    ...
    async findFiles(pattern, maxFiles) {
        const out = [];
    ...
    async scanTsconfigDrift() {
        ...
        const pick = (k) => parsed.map(p => ({ f: p.f, v: p.j?.compilerOptions?.[k] }));
        const compareSet = (arr, label, rec, sev) => {
    ...
    async scanImportExtensionConsistency() {
...
//# sourceMappingURL=systemic-scanner.js.map