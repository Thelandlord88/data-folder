...
export class HunterAudit {
    ...
    async auditBobsWork() {
        ...
        const auditAgent = await nexus.createOptimizedSession('audit header architecture for critical violations, gaps, and limitations with brutal honesty about production readiness');
        // Define audit scope
        const scope = {
            codeFiles: [
    ...
    combineAuditResults(enhancedAnalysis, policyAudit) {
    ...
    generateHunterReport(auditResults) {
...
`;
        ...
        const unknowns = findings.filter((f) => f.type === 'untested' || f.type === 'assumption');
        if (unknowns.length > 0) {
            report += `
## 🔍 WHAT WE CAN'T VERIFY (HONEST LIMITATIONS):

`;
            unknowns.forEach((finding) => {
                report += `
- **${finding.description}**
...
`;
    ...
    generateNextActions(auditResults) {
...
export async function executeHunterFollowUp() {
    ...
    auditResults.nextActions.forEach((action, index) => {
...
export class MockFileSystem {
    ...
    static async isTokenDefined(tokenName) {
...
// Main execution when run directly (not when imported)
if (import.meta.url === `file://${process.argv[1]}` && process.argv[1]?.includes('Hunter.audit')) {
...
//# sourceMappingURL=Hunter.audit.js.map