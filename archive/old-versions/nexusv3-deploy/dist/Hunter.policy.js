...
export class HunterPolicy {
    traits = new Map();
    evidenceChain = [];
    tokenDefinitions = new Set();
    tokenScopeSignature = null;
    constructor() {
     ...
     */
    async verifyEvidence(claim, scope) {
        const findings = [];
     ...
     */
    async detectArchitecturalViolations(scope) {
        const findings = [];
        for (const file of scope.codeFiles) {
            const content = await this.readFile(file);
    ...
    async ensureTokenDefinitions(configFiles) {
    ...
    isTokenDefined(tokenName) {
        return this.tokenDefinitions.has(tokenName);
    }
    findDuplicatePatterns(items) {
...
//# sourceMappingURL=Hunter.policy.js.map