...
import type {
  ...
  RegistryStatus,
  CognitiveTrait,
} from '../types/personality.types.js';
...
class MetricsCollector {
  ...
  recordError(personalityName: string): void {
  ...
  getPerformanceReport(): RegistryMetrics {
    return {
...
export class PersonalityRegistryLoader {
  ...
  private personalityRegistry: Map<string, PersonalityData> = new Map();
  private metrics: MetricsCollector;
  private circuitBreaker: CircuitBreaker;
  private cache?: LRUCache<string, PersonalityData>;
  ...
  constructor(config: RegistryConfig = {}) {
    ...
    if (this.config.cacheEnabled) {
      this.cache = new LRUCache<string, PersonalityData>({
  ...
  private async discoverPersonalityFiles(): Promise<string[]> {
    try {
  ...
  private async discoverAndLoadPersonalities(): Promise<void> {
  ...
  private async loadPersonalityWithCircuitBreaker(personalityName: string): Promise<PersonalityData | null> {
    return this.circuitBreaker.execute(() => this.loadPersonality(personalityName));
  }

  private async loadPersonality(personalityName: string): Promise<PersonalityData | null> {
    ...
    try {
      ...
      const personalityData = JSON.parse(fileContent) as PersonalityData;
      ...
      if (this.validatePersonalityData(personalityData)) {
        ...
        console.log(
          `  ✅ Loaded ${sanitizedPersonalityName} (${Object.keys(personalityData.cognitiveTraits || {}).length} traits)`
        );
  ...
  private sanitizePersonalityName(personalityName: string): string {
  ...
  private validatePersonalityData(data: unknown): boolean {
    if (!this.config.validationStrict) {
      // Basic validation for non-strict mode
      const basicData = data as Partial<PersonalityData>;
  ...
  analyzeRegistry(): RegistryAnalysis {
    ...
    const compliantPersonalities = analysis.personalityBreakdown.filter(
      (p) => p.hasRiskPolicy && p.hasRequiredSections
    );
    analysis.validationCompliance = Math.round(
      (compliantPersonalities.length / analysis.totalPersonalities) * 100
    );
  ...
  getAutoGenerationCapabilities(): AutoGenerationCapability[] {
    const capabilities: AutoGenerationCapability[] = [];

    for (const [name, data] of this.personalityRegistry) {
      const traits = data.cognitiveTraits || {};
      const principles = data.ideology?.principles || [];

      const capability: AutoGenerationCapability = {
        personality: name,
  ...
  private calculateAverageExpertise(traits: Record<string, CognitiveTrait>): number {
  ...
  private extractKnowledgeDomains(traits: Record<string, CognitiveTrait>): string[] {
  ...
  private getHealthWarnings(analysis: RegistryAnalysis, metrics: RegistryMetrics, registrySize: number): string[] {
