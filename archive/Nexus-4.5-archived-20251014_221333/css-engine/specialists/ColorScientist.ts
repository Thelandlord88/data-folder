...
import { hexToOKLCH, generateRamp, oklchToCSS, type OKLCHColor } from '../utils/oklch.js';
import { auditRamp, generateAccessiblePairs, type WCAGLevel } from '../utils/contrast.js';
 ...
 */
export class ColorScientist implements Specialist<DesignDNA, ColorFacts> {
  ...
  async run(dna: DesignDNA, opts: CompileOptions): Promise<ColorFacts> {
    ...
    try {
      ...
      throw new Error(`ColorScientist failed: ${(error as Error).message}`);
    }
  }
   ...
   */
  private extractPrimary(colors?: string[]): string {
   ...
   */
  private detectHarmony(color: OKLCHColor): 'triadic' | 'complementary' | 'analogous' {
    // Simple heuristic based on hue
    // This is just metadata - doesn't affect generation
    const hue = color.h;
    
    if (hue >= 0 && hue < 60) return 'triadic';
    if (hue >= 60 && hue < 180) return 'analogous';
 ...
 */
export function createColorScientist(): ColorScientist {
