...
export class WcagHunterService {
    ...
    async checkHtml(html) {
        ...
        return new Promise((resolve, reject) => {
            ...
            let stdout = '';
            let stderr = '';
            hunter.stdout.on('data', (data) => {
                stdout += data.toString();
            });
            hunter.stderr.on('data', (data) => {
     ...
     */
    async getPersonalityAnalysis(report) {
     ...
     */
    async toIntelligenceData(report) {
     ...
     */
    async getStrategicImplications(report) {
        const implications = {
            businessImpact: [],
            riskFactors: [],
            opportunities: [],
        ...
        if (report.total_issues > 0) {
            implications.accessibilityImpact.push('Vision-impaired users may face barriers', 'Screen reader compatibility issues detected');
            implications.legalRisks.push('Non-compliance with WCAG may expose to legal action', 'ADA/Section 508 requirements not fully met');
            implications.businessImpact.push(`Estimated fix time: ${report.summary.estimated_fix_time_minutes} minutes`, 'User base excludes people with disabilities');
            implications.userExperienceImpact.push('Some users cannot access content', 'Navigation may be difficult without mouse');
        }
        else {
        ...
        if (report.summary.wcag_aa_compliance === 'fully-compliant') {
            implications.seoOpportunities.push('Semantic structure improves SEO', 'Alt text provides content for search engines');
        }
        return implications;
    }
    /**
...
//# sourceMappingURL=wcag-hunter.service.js.map