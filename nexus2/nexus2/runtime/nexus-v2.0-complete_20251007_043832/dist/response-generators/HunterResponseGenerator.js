...
export class HunterResponseGenerator {
    generateResponse(request, personality, principles, context) {
        ...
        const content = this.synthesizeForensicResponse(request, evidenceAudit, riskAssessment, verificationPlan, personality.identity?.name || 'Hunter');
        return {
            content,
    ...
    calculateConfidence(request, domain) {
        ...
        if (domain === 'forensic') {
            if (this.containsEvidenceTerms(request))
                confidence += 0.25;
            if (this.containsRiskIndicators(request))
                confidence += 0.15;
            if (this.containsVerificationNeeds(request))
                confidence += 0.1;
        }
    ...
    synthesizeForensicResponse(request, evidenceAudit, riskAssessment, verificationPlan, personalityName) {
        const evidenceSection = evidenceAudit.length > 0
            ? `### 🔍 Evidence Audit\n${evidenceAudit.map(e => `- **${e.claim}**: ${e.evidenceLevel} evidence (${e.verification})`).join('\n')}\n\n`
            : '';
        const riskSection = riskAssessment.length > 0
            ? `### ⚠️ Risk Assessment\n${riskAssessment.map(r => `- **${r.risk}**: ${r.probability} probability, ${r.impact} impact\n  *Mitigation: ${r.mitigation}*`).join('\n')}\n\n`
            : '';
        const verificationSection = verificationPlan.length > 0
            ? `### ✅ Verification Requirements\n${verificationPlan.map(v => `- **${v.requirement}**: ${v.method} (${v.priority} priority, ${v.effort} effort)`).join('\n')}\n\n`
            : '';
...
*This analysis applies systematic forensic methodology to identify evidence gaps, assess risks, and establish verification requirements for reliable conclusions.*`;
    ...
    assessEvidenceLevel(claim) {
        ...
        if (claim.text.includes('test') || claim.text.includes('measure'))
            return 'strong';
        if (claim.text.includes('benchmark') || claim.text.includes('profile'))
            return 'moderate';
        if (claim.text.includes('should') || claim.text.includes('will'))
            return 'weak';
        return 'none';
    }
    ...
    assessClaimCredibility(claim) {
        let credibility = 0.5;
        if (claim.text.includes('test') || claim.text.includes('benchmark'))
            credibility += 0.3;
        if (claim.text.includes('should') || claim.text.includes('might'))
            credibility -= 0.2;
        if (claim.text.includes('always') || claim.text.includes('never'))
            credibility -= 0.3;
    ...
    identifyRiskSurfaces(request) {
        const risks = [];
        if (/network|api|service/i.test(request))
            risks.push('Network reliability');
        if (/database|storage|data/i.test(request))
            risks.push('Data integrity');
        if (/user|input|form/i.test(request))
            risks.push('Input validation');
        if (/deploy|production|release/i.test(request))
            risks.push('Deployment failure');
        return risks;
    }
    ...
    containsIntegrationPoints(request) {
        return /integrat|api|service|microservice|connect/i.test(request);
    }
    containsPerformanceClaims(request) {
    ...
    assessEvidenceComplexity(request) {
        let complexity = 0.4;
        if (/evidence|proof|verify/i.test(request))
            complexity += 0.4;
        if (/test|measure|benchmark/i.test(request))
            complexity += 0.2;
        return Math.min(complexity, 1.0);
    }
    assessRiskComplexity(request) {
        let complexity = 0.3;
        if (/risk|fail|security|performance/i.test(request))
            complexity += 0.5;
        if (/critical|severe|high/i.test(request))
            complexity += 0.2;
        return Math.min(complexity, 1.0);
    }
    assessHuntingOpportunity(request) {
        let opportunity = 0.5;
        if (/hunt|find|discover|identify/i.test(request))
            opportunity += 0.3;
        if (/hidden|unknown|potential|latent/i.test(request))
            opportunity += 0.2;
        return Math.min(opportunity, 1.0);
    }
    assessEvidenceQuality(request) {
        let quality = 0.5;
        if (/test|benchmark|measure|data/i.test(request))
            quality += 0.3;
        if (/proven|validated|verified/i.test(request))
            quality += 0.2;
        if (/assume|believe|think|should/i.test(request))
            quality -= 0.2;
    ...
    determineAnalysisDepth(request, evidenceAudit) {
        ...
        if (evidenceComplexity >= 3 && riskIndicators)
            return 'deep';
        if (evidenceComplexity >= 2 || riskIndicators)
            return 'moderate';
        return 'surface';
    }
...
//# sourceMappingURL=HunterResponseGenerator.js.map