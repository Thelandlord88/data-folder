...
class NexusRuntime {
  ...
  async loadResponseGenerators() {
    try {
      // Load personality registry
      const registryModule = await import('./response-generators/PersonalityRegistryLoader.mjs');
      const registryLoader = new registryModule.PersonalityRegistryLoader();
      const personalityRegistry = await registryLoader.loadAllPersonalities();
  ...
  async processRequest(personality, rawRequest) {
    ...
    if (this.responseFactory && this.responseFactory.hasSpecializedGenerator(personalityName)) {
      ...
      const generator = this.responseFactory.getGenerator(personalityName);
...
- Perform disk writes off the hot path—handlers only enqueue, the worker handles I/O.`;
  ...
  buildStatusPayload() {
    ...
    const traitCompositionStats = nexusTraitBridge.getCompositionAnalytics?.() ?? {};
